...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function drawChart(cashflows) {
    ...
    const invAngle = (investments / total) * Math.PI * 2;
    ...
    if (investments > 0) {
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.arc(cx, cy, radius, -Math.PI / 2 + gap, -Math.PI / 2 + invAngle - gap);
      ctx.closePath();
    ...
    if (returns > 0) {
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.arc(cx, cy, radius, -Math.PI / 2 + invAngle + gap, -Math.PI / 2 + invAngle + retAngle - gap);
      ctx.closePath();

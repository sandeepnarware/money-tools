document.addEventListener('DOMContentLoaded', () => {
  ...
  function drawChart(deposits, interest) {
    ...
    const depAngle = (deposits / total) * Math.PI * 2;
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, radius, -Math.PI / 2, -Math.PI / 2 + depAngle);
    ctx.closePath();
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, radius, -Math.PI / 2 + depAngle, -Math.PI / 2 + depAngle + intAngle);
    ctx.closePath();

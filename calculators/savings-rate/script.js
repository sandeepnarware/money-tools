document.addEventListener('DOMContentLoaded', () => {
  ...
  function drawChart(expenses, savings) {
    ...
    const expAngle = (expenses / total) * Math.PI * 2;
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, radius, -Math.PI / 2, -Math.PI / 2 + expAngle);
    ctx.closePath();
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, radius, -Math.PI / 2 + expAngle, -Math.PI / 2 + expAngle + savAngle);
    ctx.closePath();

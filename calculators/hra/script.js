document.addEventListener('DOMContentLoaded', () => {
  ...
  function drawChart(exempted, taxable) {
    ...
    const excAngle = (exempted / total) * Math.PI * 2;
    const taxAngle = (taxable / total) * Math.PI * 2;

    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, radius, -Math.PI / 2, -Math.PI / 2 + excAngle);
    ctx.closePath();
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, radius, -Math.PI / 2 + excAngle, -Math.PI / 2 + excAngle + taxAngle);
    ctx.closePath();

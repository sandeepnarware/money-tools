document.addEventListener('DOMContentLoaded', () => {
  ...
  function drawChart(contributions, interest) {
    ...
    const conAngle = (contributions / total) * Math.PI * 2;
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, radius, -Math.PI / 2, -Math.PI / 2 + conAngle);
    ctx.closePath();
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, radius, -Math.PI / 2 + conAngle, -Math.PI / 2 + conAngle + intAngle);
    ctx.closePath();
